 ...
 * mini event loop: await and select are the same while(!done)
 * run_loop_once() shape as the libuv backend, except run_loop_once() is
 * ours: zsock_poll() over the in-flight socket operations plus one eventfd,
